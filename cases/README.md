...
Seed (rev-0 draft): C1–C7 are codex's process-lifecycle cases from its adoption spike; C8–C18 came out of the document
gate r1 and the second pass. The `expected` column is the contract; the host columns are bookkeeping. A host test that
