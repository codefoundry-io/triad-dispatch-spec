...
rule covers narrative documentation only. Symlinks (owner Q4, OPEN): two clauses. INTEGRITY — a symlink in the reviewed basis is fingerprinted by its link TEXT (B today; A's detached worktree carries none unless a leg creates one, which A's verify then refuses). DELIVERY — the tree handed to legs must not contain a link a reader tool could follow outside it; round r2 split 2:1 between materializing each link as a regular file holding its text in the ROUND COPY with a manifest (codex, google) and a prompt-level no-follow clause (claude); the owner decides.
